...
extern struct irx_export_table	_exp_ps2split;
...
static inline int InitLWIPStack(struct ip_addr *IP, struct ip_addr *NM, struct ip_addr *GW){
	...
	if ((iRet=RegisterLibraryEntries(&_exp_ps2ip))!=0)
	{
		...
		if ((iRet=RegisterLibraryEntries(&_exp_ps2split))!=0)
		{
			printf("PS2SPLIT: RegisterLibraryEntries returned: %d\n", iRet);
			ReleaseLibraryEntries(&_exp_ps2ip);
			return iRet;
		}
