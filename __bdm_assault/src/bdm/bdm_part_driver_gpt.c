...
int part_connect_gpt(struct block_device *bd)
{
    ...
    if (ret != 1)
    {
        ...
        bdm_trace_log("PART_GPT header_read_failed ret=%d\n", ret);
    ...
    if (memcmp(pGptHeader->signature, EFI_PARTITION_SIGNATURE, sizeof(EFI_PARTITION_SIGNATURE)) != 0)
    {
        ...
        bdm_trace_log("PART_GPT invalid_signature\n");
    ...
    bdm_trace_log("PART_GPT header partitions=%u entry_lba=%08x%08x entries_per_sector=%d\n",
                  pGptHeader->partition_count,
                  ((u32 *)&pGptHeader->partition_table_lba)[1], ((u32 *)&pGptHeader->partition_table_lba)[0],
                  entriesPerSector);
    ...
    for (int i = 0; i < pGptHeader->partition_count; )
    {
        ...
        if (i % entriesPerSector == 0)
        {
            ...
            if (ret != 1)
            {
                ...
                bdm_trace_log("PART_GPT entry_read_failed index=%d ret=%d\n", i, ret);
#ifdef DEBUG
            ...
            for (int x = 0; x < entriesPerSector; x++, i++)
            {
                ...
                if (pGptPartitionEntry[x].first_lba < pGptHeader->first_lba || pGptPartitionEntry[x].last_lba > pGptHeader->last_lba)
                {
                    ...
                    bdm_trace_log("PART_GPT corrupt_entry index=%d\n", i);
                ...
                bdm_trace_log("PART_GPT entry=%d name=%.36s start=%08x%08x end=%08x%08x\n",
                              i + 1, partName,
                              ((u32 *)&pGptPartitionEntry[x].first_lba)[1], ((u32 *)&pGptPartitionEntry[x].first_lba)[0],
                              ((u32 *)&pGptPartitionEntry[x].last_lba)[1], ((u32 *)&pGptPartitionEntry[x].last_lba)[0]);
                ...
                if ((partIndex = GetNextFreePartitionIndex()) == -1)
                {
                    ...
                    bdm_trace_log("PART_GPT no_free_slot entry=%d\n", i + 1);
    ...
    FreeSysMemory(buffer);
    bdm_trace_log("PART_GPT complete partitions=%d ret=%d\n", mountCount, mountCount > 0 ? 0 : -1);
